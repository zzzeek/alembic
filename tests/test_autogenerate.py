...
class AutogeneratePrimaryKeyTest(AutogenTest, TestCase):
    ...
    def _get_db_schema(cls):
        ...
        Table('order', m,
            ...
            Column('amount', Numeric(10, 2), nullable=True),
            Column('user_id', Integer, ForeignKey('user.id')),
            PrimaryKeyConstraint('order_id', name='order_pkey')
        )

        Table('extra', m,
            Column("x", CHAR),
            Column('uid', Integer, ForeignKey('user.id')),
            PrimaryKeyConstraint('x', name='extras_pkey')
        )
        ...
        Table('item', m,
            ...
            Column('description', String(100)),
            Column('order_id', Integer, ForeignKey('order.order_id')),
            UniqueConstraint('id', 'order_id', name='item_order_unique')
        )
    ...
    def _get_model_schema(cls):
        ...
        Table('user', m,
            ...
            Column('active', Boolean),
            Column('name', String(50), nullable=False),
            # PK column change of id -> id, active
            PrimaryKeyConstraint('id', 'active', name='user_pkey')
        )

        Table('address', m,
            Column('id', Integer, nullable=False),
            Column('email_address', String(100), nullable=False),
            # Explicit PK is identical in implementation to m2's quick PK,
            # but creates drop and creates because the conn's PK
            # doesn't have an explicit name
        ...
        Table('order', m,
            ...
            Column('amount', Numeric(10, 2), nullable=True),
            Column('user_id', Integer, ForeignKey('user.id'))
            # Quick PK is identical in implementation to m2's explicit PK,
            # and there are no DDL events because the conn's explicit name
            # is identical to the metadata's autogenerated PK name
        )
        ...
        Table('extra', m,
            ...
            Column('uid', Integer, ForeignKey('user.id')),
            # PK's name changes
            PrimaryKeyConstraint('x', name='extra_pkey')
        )
        ...
        Table('item', m,
            ...
            Column('description', String(100)),
            Column('order_id', Integer, ForeignKey('order.order_id')),
            # Quick PK and an explicit UQ are dropped and a composite
            # PK is added
            PrimaryKeyConstraint('id', 'order_id', name='item_order_pkey')
        )
    ...
    def test_diffs(self):
        ...
        eq_(type(diffs[0][1]), PrimaryKeyConstraint)
        eq_(diffs[0][1].table.name, "address")
        eq_(diffs[0][1].name, None)

        eq_(diffs[1][0], "add_constraint")
        eq_(type(diffs[1][1]), PrimaryKeyConstraint)
        eq_(diffs[1][1].table.name, "address")
        eq_(diffs[1][1].name, "address_pkey")

        eq_(diffs[2][0], "remove_constraint")
        eq_(type(diffs[2][1]), PrimaryKeyConstraint)
        eq_(diffs[2][1].table.name, "extra")
        eq_(diffs[2][1].name, None)

        eq_(diffs[3][0], "add_constraint")
        eq_(type(diffs[3][1]), PrimaryKeyConstraint)
        eq_(diffs[3][1].table.name, "extra")
        eq_(diffs[3][1].name, "extra_pkey")

        eq_(diffs[4][0], "remove_constraint")
        eq_(type(diffs[4][1]), PrimaryKeyConstraint)
        eq_(diffs[4][1].table.name, "item")
        eq_(diffs[4][1].name, None)

        eq_(diffs[5][0], "add_constraint")
        eq_(type(diffs[5][1]), PrimaryKeyConstraint)
        eq_(diffs[5][1].table.name, "item")
        eq_(diffs[5][1].name, "item_order_pkey")
        eq_([col.name for col in diffs[5][1].columns], ["id", "order_id"])

        eq_(diffs[6][0], "remove_constraint")
        eq_(type(diffs[6][1]), UniqueConstraint)
        eq_(diffs[6][1].table.name, "item")
        eq_(diffs[6][1].name, "item_order_unique")

        eq_(diffs[7][0], "remove_constraint")
        eq_(type(diffs[7][1]), PrimaryKeyConstraint)
        eq_(diffs[7][1].table.name, "user")
        eq_(diffs[7][1].name, None)

        eq_(diffs[8][0], "add_constraint")
        eq_(type(diffs[8][1]), PrimaryKeyConstraint)
        eq_(diffs[8][1].table.name, "user")
        eq_(diffs[8][1].name, "user_pkey")
        eq_([col.name for col in diffs[8][1].columns], ["id", "active"])

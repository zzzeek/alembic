...
class AutogenerateDiffTest(ModelOne, AutogenTest, TestCase):
    ...
    def test_diff_only_compare_listed_tables(self):
        metadata = self.m2
        connection = self.context.bind
        diffs = []

        autogenerate._produce_net_changes(
            connection, metadata, diffs, self.autogen_context,
            object_filters=_default_object_filters, only=['user']
        )

        eq_(diffs[0][0], 'remove_column')
        eq_(diffs[0][3].name, 'pw')

        eq_(diffs[1][0][0], "modify_default")
        eq_(diffs[1][0][1], None)
        eq_(diffs[1][0][2], "user")
        eq_(diffs[1][0][3], "a1")
        eq_(diffs[1][0][6].arg, "x")

        eq_(diffs[2][0][0], 'modify_nullable')
        eq_(diffs[2][0][5], True)
        eq_(diffs[2][0][6], False)

    def test_diff_only_compare_tables_satisfying_a_predicate(self):
        metadata = self.m2
        connection = self.context.bind
        diffs = []
        ...
        autogenerate._produce_net_changes(
            connection, metadata, diffs, self.autogen_context,
            object_filters=_default_object_filters,
            only=lambda table_name: table_name == 'address'
        )

        eq_(diffs[0][0], "add_column")
        eq_(diffs[0][1], None)
        eq_(diffs[0][2], "address")
        eq_(diffs[0][3], metadata.tables['address'].c.street)

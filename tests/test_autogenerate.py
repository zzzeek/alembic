...
class AutogenerateDiffTestModelFive(AutogenTest, TestCase):
    ...
    def test_diffs(self):
        ...
        eq_(diffs[1][1].name, "order_order_id_user_id_unique")

        eq_(diffs[2][0], "remove_index")
        eq_(diffs[2][1].name, "order_user_id_amount_idx")
        eq_(diffs[2][1].unique, False)

        eq_(diffs[3][0], "add_index")
        eq_(diffs[3][1].name, "order_user_id_amount_idx")
        eq_(diffs[3][1].unique, True)

        eq_(diffs[4][0], "add_constraint")
        eq_(diffs[4][1].table.name, "user")

        eq_(diffs[5][0], "remove_index")
        eq_(diffs[5][1].name, "ix_user_name")
